...
class VerificationRunner:
    ...
    async def arun_verification(
        ...
        if build_command:
            b_res = await self.sandbox.arun_command(build_command)
        ...
        if lint_command:
            l_res = await self.sandbox.arun_command(lint_command)
            if l_res.exit_code != 0:
                linter_passed = False

        for cmd in test_commands:
            t_res = await self.sandbox.arun_command(cmd)
    ...
    async def evaluate_reproduction_async(
        self,
        repro_script: str,
        ...
        for cmd in pre_patch_test_commands:
            res = await self.sandbox.arun_command(cmd)
            if res.exit_code != 0:
                failed_on_base = True
                break
    ...
    async def full_verification_pipeline_async(
        self,
        test_commands: List[str],
        repro_script: str,
        pre_patch_test_commands: List[str],
        ...
        v_result = await self.arun_verification(test_commands, build_command, lint_command)

        repro = await self.evaluate_reproduction_async(repro_script, pre_patch_test_commands, post_patch_test_commands)
        ...
        if decision not in (VerificationDecision.AUTO_MERGE, VerificationDecision.HUMAN_REVIEW):
            v_result.overall_success = False
            if decision == VerificationDecision.REJECT_REPRO_MISSING and not v_result.failure_reason:
                v_result.failure_reason = "Reproduction test flip not confirmed"
            elif decision == VerificationDecision.SECURITY_HOLD and not v_result.failure_reason:
                v_result.failure_reason = "High or critical SAST security findings in patch"
            elif decision == VerificationDecision.REJECT_BUILD_FAILURE and not v_result.failure_reason:
                v_result.failure_reason = "Build failure detected"
            elif decision == VerificationDecision.REJECT_REGRESSION and not v_result.failure_reason:
                v_result.failure_reason = "Regression test failure detected"
