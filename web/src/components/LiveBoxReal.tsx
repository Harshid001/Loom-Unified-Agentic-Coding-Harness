  ...
  const handleStart = async () => {
    ...
    try {
      const response = await fetch("/api/run", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ issue, model, repo_path: repoPath, mock: mockMode }),
      });
      if (!response.ok) {
          ...
          <section className="min-h-0 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-4 lg:col-span-1 flex flex-col justify-between">
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                {steps.map(step => (
                  <div key={step.name} className="flex items-center justify-between rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-surface)] px-3 py-1.5 text-xs font-mono">
                    <span className="text-[var(--text-secondary)] capitalize">{step.name}</span>
                    <span className={`text-[10px] uppercase font-semibold ${step.status === 'completed' ? 'text-[var(--success)]' : step.status === 'running' ? 'text-[var(--warning)]' : 'text-[var(--text-muted)]'}`}>
                      {step.status}
                    </span>
                  </div>
                ))}
