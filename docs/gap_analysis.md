...
| §3.4 Firecracker MicroVM enclaves, vsock guest agent, egress enforcement | ✅ | FirecrackerVM lifecycle (`loom/sandbox/firecracker_vm.py`), virtio-vsock guest agent (`firecracker_guest_agent.py`), HTTPS worker daemon (`firecracker_worker.py`), and fail-closed sandbox factory (`factory.py`) implemented and tested |
