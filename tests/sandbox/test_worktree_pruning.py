...
from loom.sandbox.worktree import WorktreeManager, SNAPSHOT_IGNORED_PATTERNS
...
def test_worktree_ignores_heavy_build_artifacts(tmp_path):
    ...
    manager = WorktreeManager(repo_dir, max_retained_snapshots=3)
...
def test_worktree_auto_prunes_older_snapshots(tmp_path):
    ...
    manager = WorktreeManager(repo_dir, max_retained_snapshots=2)
...
def test_cleanup_all_snapshots(tmp_path):
    ...
    manager = WorktreeManager(repo_dir, max_retained_snapshots=5)
